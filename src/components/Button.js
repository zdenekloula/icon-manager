...
const getButton = name => ({ theme, color }) => {
  ...
  const buttonStyle =
    settings[name][color] || settings["buttonColor"]["primary"];
  return buttonStyle;
};

export const StyledButton = styled(props => {
  const { element, className, children, color, ...other } = props;
  ...
  if (Component === "button" && props.href) {
    console.error(
      `<Button> component has been defined as "button" with existing "href". Either remove "href" from element change element to "a".`,
    );
  }

  if (Component === "a" && !props.href) {
    console.error(
      `<Button> component has a defined "a" tag without "href" attribute. Either add "href" to element or use element as "button".`,
    );
  }
  return (
    <Component
