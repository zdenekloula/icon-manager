...
const IconGrid = props => {
  ...
  const { projectsData, updateProjectsData, activeProject } = useContext(
    AppContext,
  );
  ...
  return (
    <IconGridContainer>
      <AutoSizer>
        {({ height, width }) => {
          let itemsPerRow =
            Math.floor(width / CARD_HEIGHT) > 4
              ? 4
              : Math.floor(width / CARD_HEIGHT) || 1;
          ...
          return (
            <List
              ...
              rowRenderer={({ index, key, style }) => {
                ...
                const toIndex = Math.min(fromIndex + itemsPerRow, icons.length);
